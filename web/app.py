...
def usi_info_to_pv(board, info):
    ...
    for usi_move in m[3].split(' '):
        ...
        if not board2.is_legal(move):
            break
        pv.append(TURN_SYMBOLS[board2.turn] +
                  KIF.move_to_kif(move, board2.peek()))
        board2.push(move)
...
def match(moves, engine1=None, engine2=None, options1={}, options2={}, names=None, byoyomi=None, time=None, inc=None, draw=256, human_input=None, csa=None):
    ...
    for engine in (engine1, engine2):
        if engine == 'human':
            ...
            try:
                eg = Engine(engine, connect=True, debug=True)
            except EOFError as e:
                print('Engine {0} returns EOF: {1}'.format(engine, e))
            engines.append(eg)
    ...
    while not is_game_over:
        ...
        listener.info = ''
        listener.bestmove = ''
        bestmove, _ = engine.go(
            byoyomi=byoyomi, btime=remain_time[BLACK], wtime=remain_time[WHITE], binc=inc, winc=inc, listener=listener)
        ...
        if bestmove == 'resign':
            ...
            if board.is_legal(move):
                if csa:
                    csa_exporter.move(move, time=int(
                        elapsed_time), comment=usi_info_to_csa_comment(board, listener.info))
...
def run(engine1: Optional[str] = None, engine2: Optional[str] = None, options1: Dict = {}, options2: Dict = {}, name1: Optional[str] = None, name2: Optional[str] = None, byoyomi: Optional[int] = None, time: Optional[int] = None, inc: Optional[int] = None, draw: int = 256, csa: Optional[str] = None, host: str = 'localhost', port: int = 8000):
    ...
    if engine1 and engine2:
        ...
        match_proc = Process(target=match, args=[
                             moves, engine1, engine2, options1, options2, names, byoyomi, time, inc, draw, human_input, csa])
        match_proc.start()
        is_match = 'true'
        auto_update = 'true'
    elif csa:
            ...
            if len(comment_items) > 1:
                ...
                for csa in comment_items[1:]:
                    ...
                    if board2.is_legal(move2):
                        pv.append(
                            CSA_TURN_SYMBOLS[csa[0]] + KIF.move_to_kif(move2))
                        board2.push(move2)
                    else:
                        pv = comment_items[1:]
                        break
    ...
    def init_board():
        ...
        if is_match == 'true':
            if match_proc.is_alive():
                autoupdate = request.args.get(
                    'autoupdate', default=auto_update)
...
def colab(engine1=None, engine2=None, options1={}, options2={}, name1=None, name2=None, byoyomi=None, time=None, inc=None, draw=256, csa=None):
    ...
    proc = Process(target=run, args=(engine1, engine2, options1, options2,
                   name1, name2, byoyomi, time, inc, draw, csa, 'localhost', port))
    proc.start()
