...
class MCTSUSI(BaseUSI):
    ...
    def usi(self):
        ...
        print('option name gpu_id type spin default ' +
              str(DEFAULT_GPU_ID) + ' min -1 max 7')
        print('option name batchsize type spin default ' +
              str(DEFAULT_BATCH_SIZE) + ' min 1 max 256')
        print('option name resign_threshold type spin default ' +
              str(int(DEFAULT_RESIGN_THRESHOLD * 100)) + ' min 0 max 100')
        print('option name c_puct type spin default ' +
              str(int(DEFAULT_C_PUCT * 100)) + ' min 10 max 1000')
        print('option name temperature type spin default ' +
              str(int(DEFAULT_TEMPERATURE * 100)) + ' min 10 max 1000')
        print('option name time_margin type spin default ' +
              str(DEFAULT_TIME_MARGIN) + ' min 0 max 1000')
        print('option name byoyomi_margin type spin default ' +
              str(DEFAULT_BYOYOMI_MARGIN) + ' min 0 max 1000')
        print('option name pv_interval type spin default ' +
              str(DEFAULT_PV_INTERVAL) + ' min 0 max 10000')
        ...
        if infinite or ponder:
            ...
            if self.remaining_time is None and byoyomi is None and inc is None:
                ...
                self.remaining_time = int(
                    self.remaining_time) if self.remaining_time else 0
                inc = int(inc) if inc else 0
                self.time_limit = self.remaining_time / \
                    (14 + max(0, 30 - self.root_board.move_number)) + inc
                # 秒読みの場合
                if byoyomi:
    ...
    def search(self):
        ...
        while True:
            ...
            for i in range(self.batch_size):
                ...
                result = self.uct_search(
                    board, self.tree.current_head, trajectories_batch[-1])
            ...
            for trajectories in trajectories_batch:
                ...
                for i in reversed(range(len(trajectories))):
                    ...
                    if result is None:
                        # 葉ノード
                        result = 1.0 - \
                            current_node.child_node[next_index].value
    ...
    def uct_search(self, board, current_node, trajectories):
        ...
        if not current_node.child_node:
            current_node.child_node = [
                None for _ in range(len(current_node.child_move))]
        # UCB値が最大の手を求める
    ...
    def select_max_ucb_child(self, node):
        ...
        if node.move_count == 0:
            ...
            u = np.sqrt(np.float32(node.move_count)) / \
                (1 + node.child_move_count)
    ...
    def check_interruption(self):
        ...
        rest = int(self.playout_count *
                   ((self.time_limit - spend_time) / spend_time))
    ...
    def make_input_features(self, board):
        make_input_features(board, self.features.numpy()
    ...
    def eval_node(self):
        ...
        for i, (policy_logit, value) in enumerate(zip(policy_logits, values)):
            ...
            # 合法手一覧
            legal_move_probabilities = np.empty(
                len(current_node.child_move), dtype=np.float32)
            ...
            probabilities = softmax_temperature_with_normalize(
                legal_move_probabilities, self.temperature)
